...
def imageUpload():
    ...
    classNames = ['ACNE', 'ECZEMA', 'MELANOMA SKIN CANCER', 'PSORIASIS']
    predicted_class = classNames[np.argmax(prediction)]
    predicted_class = "PREDICTED DISEASE: " + predicted_class
    confidence = round(100 * (np.max(prediction)))

    f.close()

    user = User.query.filter_by(id=current_user.id).first()
    return render_template('dashboard.html', username=user.username.upper(), prediction=predicted_class, accuracy=confidence)
